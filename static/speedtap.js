/* ==========================================================================
   ...
   Screens : home -> queue -> arena (+ final popup)
   Arena phases (data-phase on #screen-arena):
     idle       match found, waiting for round 1
     countdown  3, 2, 1
     wait       "Wait for green" (tapping now = false start)
     go         green flash, TAP NOW!
     locked     you tapped, waiting for the opponent
     foul       you tapped early
     result     round result card is showing
   ========================================================================== */
(() => {
  'use strict';
  ...
  const state = {
    ...
    phase: 'idle',
    inMatch: false,
    signalAt: null,        // performance.now() right when the green frame started
    roundsToWin: 2,
  ...
  function announce(text) {
    el.sr.textContent = '';
    // tiny delay so screen readers notice repeated messages
  ...
  function showScreen(name) {
    ...
    if (name === 'arena' && document.activeElement && document.activeElement.blur) {
      document.activeElement.blur(); // Space/Enter must never trigger a focused button
    }
  }
  ...
  function popMain() {
    ...
    void el.main.offsetWidth; // restart the CSS animation
  ...
  /* ------------------------------------------------------------------ */
  /* Tapping (the important part)                                        */
  /* ------------------------------------------------------------------ */
  function eventTime(e) {
    // e.timeStamp is when the browser received the input, which is more
    // accurate than performance.now() inside a possibly delayed handler.
  ...
  function doTap(t) {
    ...
    if (state.phase === 'wait') {
      // False start: tapped before the green signal.
      setPhase('foul');
  ...
  function showFinal(m) {
    ...
    if (m.reason === 'opponent_left') {
      ...
      el.finalReason.textContent = won ? 'You took the match. Nice reflexes.' : 'Your opponent took the match.';
    }
  ...
  socket.on('connect', () => {
    ...
    if (state.wasDisconnected) {
      ...
      // The server drops queue entries and matches when a socket disconnects.
  ...
  socket.on('tap_now', () => {
    ...
    // Start the clock on the frame that actually shows green.
